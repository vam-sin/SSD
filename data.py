...
def rb_patches(
    ...
    trainset = datasets.ImageFolder(
        root=os.path.join(data_dir, "patches/train"), transform=transform_train
    )
    testset = datasets.ImageFolder(
        root=os.path.join(data_dir, "patches/test"), transform=transform_test
    )
...
def only_rb_patches(
    ...
    if norm_layer is None: # this has to be changed
        norm_layer = transforms.Normalize(
            mean=[0.419], std=[0.348]
        )
    ...
    trainset = datasets.ImageFolder(
        root=os.path.join(data_dir, "patches/rb/train"), transform=transform_train
    )
    testset = datasets.ImageFolder(
        root=os.path.join(data_dir, "patches/rb/test"), transform=transform_test
    )
